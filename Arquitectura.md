# 🏗️ Diagrama de Arquitectura – Power Apps Sistema de Control Contractual y Presupuestal
...
flowchart TB
    ...
    %% Base de Datos
    DB[(Base de Datos)]
    DB --- Contratos[(Tabla Contratos)]
    DB --- Proveedores[(Tabla Proveedores)]
    DB --- Presupuesto[(Tabla Presupuesto)]
    DB --- Auditoria[(Tabla Auditoría)]

    %% Integraciones
    BI[Power BI]
    Mail[Correo Corporativo]
    SAP[SAP Sourcing (futuro)]
